 ...
 * @author Ravee
 */
public class CardGameWar extends Game {
    ...
    private void initializeDeck() {
        ...
        for (int rank = 2; rank <= 14; rank++) {
            for (String suit : suits) {
                Card card = new Playing(rank, suit);
...
//                System.out.println(hand.getOwner().getName()+"  "+card1.toString());
    ...
    private int getRankValue(Card card) {
        if (card instanceof Playing) {
            return ((Playing) card).getRank();
        }
        return 0;
    }
...
//        displayPlayerScores();
    ...
    public static void main(String[] args) {
        CardGameWar game = new CardGameWar();
